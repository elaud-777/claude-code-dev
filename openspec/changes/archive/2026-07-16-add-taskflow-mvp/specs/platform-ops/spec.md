...
시스템은 FastAPI 기본 제공 OpenAPI 문서(`/docs` Swagger UI, `/redoc`)를 노출해야 한다(SHALL). JWT Bearer 인증이 필요한 엔드포인트는 OpenAPI 보안 스키마로 선언되어 Swagger UI에서 토큰 입력 후 바로 호출 테스트가 가능해야 한다.
...
시스템은 회원가입, 로그인, 칸반 태스크 CRUD(생성/상태변경/삭제 권한 포함), 채팅 메시지 송수신(폴링 포함)에 대한 자동화된 테스트를 갖춰야 한다(SHALL). 전면적인 테스트 자동화는 요구하지 않으며, 그 외 시나리오는 수동 동작 확인으로 검증한다.

#### Scenario: 핵심 플로우 테스트 실행
- **WHEN** 개발자가 백엔드 테스트 스위트(pytest)를 실행하면
